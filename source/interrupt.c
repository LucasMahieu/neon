...
void interruptSetPriority(uint32_t irq, uint32_t priority)
{
	...
	if(irq < 32 && irq > 0 && priority > 0 && priority < 4)
	{
		...
		if(irq > 3 && irq < 8) //this irq priority resides in NVIC_IPR0
		{
			NVIC_IPR1 = priority << shiftValue;
		}

		if(irq > 7 && irq < 12) //this irq priority resides in NVIC_IPR0
		{
			NVIC_IPR2 = priority << shiftValue;
		}

		if(irq > 11 && irq < 16) //this irq priority resides in NVIC_IPR0
		{
			NVIC_IPR3 = priority << shiftValue;
		}

		if(irq > 15 && irq < 20) //this irq priority resides in NVIC_IPR0
		{
			NVIC_IPR4 = priority << shiftValue;
		}

		if(irq > 19 && irq < 24) //this irq priority resides in NVIC_IPR0
		{
			NVIC_IPR5 = priority << shiftValue;
		}

		if(irq > 23 && irq < 28) //this irq priority resides in NVIC_IPR0
		{
			NVIC_IPR6 = priority << shiftValue;
		}

		if(irq > 27 && irq < 32) //this irq priority resides in NVIC_IPR0
		{
